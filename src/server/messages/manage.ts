...
export const manageRoutes = new Elysia()
  .get(
    ...
    ({ params: { deviceId }, set }) => {
      try {
        ...
        return { success: true, data: messageStore.listChatJids(deviceId) };
      } catch (error) {
        set.status = 400;
        return {
          success: false,
    ...
    },
    {
      ...
      detail: {
        ...
        description: 'Get all unique chat JIDs with last message.',
      },
    },
  )
  .get(
    ...
    },
    {
      ...
      detail: {
        ...
        description: 'Get messages for a specific chat JID.',
      },
    },
  )
  .get(
    ...
    async function* ({ params: { deviceId, chatJid }, set }) {
      ...
      let lastCount = 0;
      yield sse(
        JSON.stringify({ type: 'connected', sessionId: deviceId, chatJid }),
      );
      while (true) {
        try {
          const messages = messageStore.listMessages(deviceId, chatJid, 200, 0);
          if (messages.length !== lastCount) {
            lastCount = messages.length;
            yield sse(
              JSON.stringify({ type: 'messages_update', data: messages }),
            );
          }
        } catch {
          yield sse(
    ...
    },
    {
      ...
      detail: {
        summary: 'Per-Chat SSE',
        description: 'Real-time message updates per chat.',
      },
    },
  )
  .get(
    ...
    },
    {
      ...
      detail: {
        summary: 'Session SSE',
        description: 'Real-time message updates for a session.',
      },
    },
  )
  .post(
  ...
  .post(
    ...
    async ({ params: { deviceId }, body, set }) => {
      try {
        const whatsapp = getWA(deviceId);
        const raw = messageStore.get(deviceId, body.messageKey) as any;
        if (!raw?.message) {
          set.status = 404;
